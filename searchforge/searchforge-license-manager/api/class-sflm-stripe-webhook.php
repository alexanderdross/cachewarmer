...
class SFLM_Stripe_Webhook extends SFLM_REST_Controller {
    ...
    public function handle( \WP_REST_Request $request ): \WP_REST_Response {
        ...
        $secret    = defined( 'SFLM_STRIPE_WEBHOOK_SECRET' ) ? SFLM_STRIPE_WEBHOOK_SECRET : '';
     ...
     */
    private function handle_checkout_completed( object $session, SFLM_License_Manager $manager ): ?int {
        global $wpdb;
        $prefix = $wpdb->prefix . SFLM_DB_PREFIX;
     ...
     */
    private function handle_payment_succeeded( object $invoice, SFLM_License_Manager $manager ): ?int {
     ...
     */
    private function handle_payment_failed( object $invoice, SFLM_License_Manager $manager ): ?int {
        if ( empty( $invoice->subscription ) ) {
            return null;
        }
     ...
     */
    private function handle_subscription_deleted( object $subscription, SFLM_License_Manager $manager ): ?int {
     ...
     */
    private function handle_subscription_updated( object $subscription, SFLM_License_Manager $manager ): ?int {
        $license = $manager->find_by_subscription( $subscription->id );
        if ( ! $license ) {
            return null;
        }
     ...
     */
    private function handle_refund_or_dispute( object $charge, SFLM_License_Manager $manager ): ?int {
        global $wpdb;
        $prefix = $wpdb->prefix . SFLM_DB_PREFIX;
    ...
    private function verify_signature( string $payload, ?string $sig_header, string $secret ): bool {
        ...
        // Autoloader wird zentral in searchforge-license-manager.php geladen
        ...
        $tolerance = defined( 'SFLM_STRIPE_WEBHOOK_TOLERANCE' ) ? (int) SFLM_STRIPE_WEBHOOK_TOLERANCE : 300;
