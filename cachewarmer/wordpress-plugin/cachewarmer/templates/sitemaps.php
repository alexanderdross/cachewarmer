...
/**
 * Translate a cron expression into a human-readable description.
 *
 * Handles the patterns generated by the plugin's own scheduler:
 *   0 * * * *         → Hourly
 *   0 3 * * *         → Daily at 03:00
 *   0 3,15 * * *      → Every 12 h starting 03:00
 *   0 0,6,12,18 * * * → Every 6 h starting 00:00
 *   (anything else)   → returns the raw expression
 */
function cachewarmer_describe_cron( ?string $cron ): string {
    if ( empty( $cron ) ) {
        return '';
    }
    if ( '0 * * * *' === $cron ) {
        return __( 'Every hour, on the hour', 'cachewarmer' );
    }
    if ( preg_match( '/^0 (\S+) \* \* \*$/', $cron, $m ) ) {
        $parts = explode( ',', $m[1] );
        $pad   = function ( $n ) {
            return str_pad( (int) $n, 2, '0', STR_PAD_LEFT );
        };
        if ( 1 === count( $parts ) ) {
            /* translators: %s: time like 03:00 */
            return sprintf( __( 'Daily at %s', 'cachewarmer' ), $pad( $parts[0] ) . ':00' );
        }
        if ( 2 === count( $parts ) ) {
            /* translators: %s: starting time like 03:00 */
            return sprintf( __( 'Every 12 h starting %s', 'cachewarmer' ), $pad( min( $parts ) ) . ':00' );
        }
        if ( 4 === count( $parts ) ) {
            /* translators: %s: starting time like 00:00 */
            return sprintf( __( 'Every 6 h starting %s', 'cachewarmer' ), $pad( min( $parts ) ) . ':00' );
        }
    }
    return $cron;
}
                ...
                <?php else : ?>
                    <?php foreach ( $sitemaps as $sitemap ) : ?>
                        <tr data-sitemap-id="<?php echo esc_attr( $sitemap->id ); ?>">
                            ...
                            <td>
                                <?php if ( $sitemap->cron_expression ) :
                                    $cron_desc = cachewarmer_describe_cron( $sitemap->cron_expression );
                                ?>
                                    <span title="<?php echo esc_attr( $sitemap->cron_expression ); ?>">
                                        <?php echo esc_html( $cron_desc ); ?>
                                    </span>
                                    <code class="cachewarmer-cron-raw"><?php echo esc_html( $sitemap->cron_expression ); ?></code>
                                <?php else : ?>
                                    <em><?php esc_html_e( 'None', 'cachewarmer' ); ?></em>
                                <?php endif; ?>
                            </td>
                            <td>
